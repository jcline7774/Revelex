...
NOTE: This version uses the Overpass API via the overpy.Overpass() library.  It should return the same results as a direct http request.
"""
...
def find_roadway(roadway_name, bbox=None):
    ...
    for way in res.ways:
        for node in way.nodes:
            gpsx.append((float(node.lon), float(node.lat)))
    return gpsx
...
def search_zone(bbox):
    ...
    # Convert overpy objects to dict format
